    ...
    with torch.no_grad():
        for layer in model.layers:
            ...
            if layer.alpha < 0.: #projecting alpha to [0,2]
                layer.alpha = nn.Parameter(torch.tensor(0.01))
            if layer.alpha > 1.:
                layer.alpha = nn.Parameter(torch.tensor(0.99))
...
# DRS layer consisting of prox_f and proximal map of wavelet regularizer
class drslayer(nn.Module):
    def __init__(self, prox_in, wavefamily = "db4", levels=5, grad_in=0.1):
        super().__init__()
    ...
    def forward(self, x, y):
        ...
        return 2*self.xinvfwt((ylnew, yhnew)) - x_inter1
...
class nonexpansivenn(nn.Module):
    def __init__(self, prox_in, wavefamilyset, levels=5, grad_in = 0.1, num_of_layers = 10):
        ...
        self.layers = nn.ModuleList([drslayer(prox_in, "db4", levels, grad_in)])
        for i in range(num_of_layers-1):
            for wave in wavefamilyset:
                self.layers.append(drslayer(prox_in, wave, levels, grad_in))
       ...
       # validate after every 5 epochs
        ...
        if (epoch + 1) % 5 == 0:
            ...
            for j, val_data in enumerate(loader_val, 0):
                val_noise = torch.FloatTensor(val_data.size()).normal_(mean=0, std=5./255.) #Generating noise
            ...
            act_str = saved_model_path + "denoise_DRS_sigma_" + str(sigma) + "_epoch_" + str(epoch+1) +".pth"
